(function () {
  ...
  var API_BASES = ["https://zh.myavlive.com"];
  var REFERER = "https://zh.myavlive.com/";
  ...
  async function getJSON(url, timeout) {
    ...
    if (/<html[\s>]|cloudflare|cf-chl-|just a moment|access denied/i.test(text)) {
      throw Host.makeError("UPSTREAM", "Myavlive 返回了访问拦截页", { url: url });
    }
    try { return JSON.parse(text); }
    catch (_) { throw Host.makeError("PARSE", "Myavlive API 返回了无效 JSON", { url: url }); }
  }
  ...
  async function currentAPI(path, timeout) {
    ...
    throw last || Host.makeError("NETWORK", "Myavlive API 请求失败", { path: path });
  }
  ...
  // test.js 里有断言锁住两者同源。
  var ROOM_PAGE_BASE = "https://zh.myavlive.com/";
  ...
  function failureReason(error) {
    ...
    if (code === "NETWORK") return "连不上 Myavlive 接口，可能是网络或节点问题，请稍后重试";
  ...
  async function resolveQualities(streamId) {
    ...
    throw last || Host.makeError("UPSTREAM", "没有可用的 Myavlive 线路", { streamId: streamId });
  }
  ...
  function parseShare(value) {
    ...
    var match = text.match(/(?:https?:\/\/)?(?:[a-z]+\.)?myavlive\.com\/([^\s/?#]+)/i);
  ...
  globalThis.LiveParsePlugin = {
    ...
    getCategories: function () {
      return [{
        ...
        subList: CATEGORIES.map(function (item) {
          return { id: item.id, parentId: "myavlive", title: item.title, icon: item.icon, biz: item.tag };
        })
      }];
    },
    ...
    getPlayback: async function (input) {
      ...
      return [{
        cdn: "myavlive-official",
        displayName: "Myavlive 官方线路",
    ...
    resolveShare: async function (input) {
      ...
      if (!username) throw Host.makeError("INVALID_ARGUMENT", "无法识别 Myavlive 分享链接", {});
