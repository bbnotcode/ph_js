    ...
    http: {
      async request(options) {
        ...
        if (url.includes("/api/front/v2/models/username/")) {
          assert.ok(url.startsWith("https://zh.myavlive.com/"), "cam API 必须使用 Myavlive 域名");
          camRequests += 1;
        ...
        if (url.includes("/api/front/v2/models?")) {
          assert.ok(url.startsWith("https://zh.myavlive.com/"), "v2 列表 API 必须使用 Myavlive 域名");
          return { status: 200, bodyText: JSON.stringify({ blocks: [] }) };
        }
        if (url.includes("/api/front/models?")) {
          assert.ok(url.startsWith("https://zh.myavlive.com/"), "列表 API 必须使用 Myavlive 域名");
...
(async () => {
  ...
  assert.strictEqual(proxied[0].displayName, "Myavlive 官方线路");
  assert.strictEqual(proxied[0].qualitys[0].liveType, "myavlive");
  ...
  assert.ok(template.indexOf("zh.myavlive.com") >= 0,
    "复制链接必须使用 Myavlive 中文站域名：" + template);
  assert.ok(template.indexOf("{userId}") >= 0, "模板必须带 {userId} 占位符：" + template);
  assert.strictEqual(template.replace("{userId}", "kem_2000"), "https://zh.myavlive.com/kem_2000");
  assert.strictEqual(JSON.stringify(manifest.shareResolve.hosts),
    JSON.stringify(["myavlive.com", "zh.myavlive.com"]),
    "分享口令要同时认两个域名的链接");

  const share = await plugin.resolveShare({ shareCode: "https://zh.myavlive.com/demo_user" });
  assert.strictEqual(share.userId, "demo_user");
  console.log("AngelLive Myavlive contract tests passed");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
