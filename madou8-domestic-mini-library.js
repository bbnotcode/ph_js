...
const MADOU8_ENTRY = '/domestic/zh-CN';
...
const WidgetMetadata = {
  ...
  description: '麻豆视频国产区自定义媒体库，支持首页、分类、搜索、详情、画质选择和 HLS 播放。'
};
...
const SECTIONS = [
  ...
  { id: '4k', title: '4K', path: 'videos/search/4K', style: 'discover.posterCompact' },
...
function parseCards(html, ctx) {
  ...
  for (let i = 0; i < starts.length; i++) {
    ...
    const href = decodeHTML(firstMatch(block, /href=["']([^"']*\/video\/(?:vid|cid)\/[^"'?#]+)["']/i));
    if (!href) continue;
...
function detailURL(ctx) { const direct = firstNonEmpty(ctx && ctx.detailUrl, ctx && ctx.url, decodeVersionId(ctx && ctx.versionId).url); if (/^https?:\/\//i.test(direct)) return direct; const id = firstNonEmpty(ctx && ctx.itemId, ctx && ctx.id, decodeVersionId(ctx && ctx.versionId).slug); return id ? entryURL(ctx) + '/video/vid/' + encodeURIComponent(String(id).replace(/^.*\//, '')) : ''; }
...
function videoSlug(url) { const m = String(url || '').match(/\/video\/(?:vid|cid)\/([^/?#"']+)/i); return m ? decodeURIComponent(m[1]) : ''; }
